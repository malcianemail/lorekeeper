<div class="col-md-4 col-6 mb-3 sprite-sort-item" data-id="{{ $sprite->id }}">
    <div class="card h-100 character-sprite-card {{ $character->active_sprite_id == $sprite->id ? 'border-primary' : '' }}">
        <div class="card-body p-2 text-center">
            @if($sprite->imageUrl)
                <a href="{{ $sprite->imageUrl }}" data-lightbox="sprites" data-title="{{ $sprite->displayName }}">
            ...
            @if($canManage)
                <div class="btn-group btn-group-sm mb-2" role="group">
                    @if($character->active_sprite_id != $sprite->id && $sprite->has_image)
                        {!! Form::open(['url' => $character->url . '/sprites/' . $sprite->id . '/active', 'class' => 'd-inline']) !!}
                    ...
                    {!! Form::open(['url' => $character->url . '/sprites/' . $sprite->id . '/delete', 'class' => 'd-inline', 'onsubmit' => 'return confirm("Delete this sprite? This cannot be undone.")']) !!}
                        {!! Form::submit('Delete', ['class' => 'btn btn-outline-danger btn-sm']) !!}
                    {!! Form::close() !!}
                </div>

                <div class="collapse text-left" id="sprite-edit-{{ $sprite->id }}">
                    {!! Form::open(['url' => $character->url . '/sprites/' . $sprite->id . '/edit', 'files' => true]) !!}
